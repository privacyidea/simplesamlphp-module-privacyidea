...
class PrivacyideaAuthSource extends UserPassBase
{
    ...
    public static function authSourceLogin(string $stateId, array $formParams): void
    {
        ...
        if (!empty($source->authSourceConfig))
        {
            ...
            {
                $headersToForward = Utils::getHeadersToForward($source->authSourceConfig['forwardHeaders']);
            }
        }
            ...
            if (!empty($username))
            {
                if (!empty($source->authSourceConfig) && !empty($source->pi))
                {
                    if (!array_key_exists('authenticationFlow', $source->authSourceConfig))
                    {
                        ...
                        try
                        {
                            $response = $source->pi->validateCheck($username, $password, $headersToForward);
                        }
                        catch (Exception $e)
                        {
                    ...
                    else
                    {
                        ...
                        if ($authenticationFlow === 'triggerChallenge')
                        {
                            if ($source->pi->serviceAccountAvailable())
                            {
                                try
                                {
                                    $response = $source->pi->triggerChallenge($username, $headersToForward);
                                }
                                catch (Exception $e)
                                {
                                    Utils::handlePrivacyIDEAException($e, $state);
        ...
        elseif ($step > 1)
        {
            try
            {
                $response = Utils::authenticatePI($state, $formParams, $headersToForward);
            }
            catch (Exception $e)
            {
                Utils::handlePrivacyIDEAException($e, $state);
